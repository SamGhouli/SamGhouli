'use client'

import { useSearchParams } from 'next/navigation'
import { TopBar } from '@/components/shared/TopBar'
import { EmptyState } from '@/components/shared/EmptyState'
import { FlagTag } from '@/components/shared/FlagTag'
import { DEMO_DATA } from '@/lib/demo/data'
import type { AcademicRecord, User } from '@/types/database'
import { BookOpen, ShieldCheck, AlertTriangle, XCircle } from 'lucide-react'

interface AcademicRow extends AcademicRecord {
  athlete: User
}

function buildDemoRows(): AcademicRow[] {
  return DEMO_DATA.academicRecords.map((rec) => ({
    ...rec,
    athlete: DEMO_DATA.athletes.find((a) => a.id === rec.athlete_id)!,
  }))
}

const ELIGIBILITY_CONFIG = {
  ok: {
    label: 'Eligible',
    icon: ShieldCheck,
    className: 'bg-green/10 border-green/20 text-green',
    cardClass: 'text-green',
  },
  review: {
    label: 'Under Review',
    icon: AlertTriangle,
    className: 'bg-amber/10 border-amber/20 text-amber',
    cardClass: 'text-amber',
  },
  ineligible: {
    label: 'Ineligible',
    icon: XCircle,
    className: 'bg-rose/10 border-rose/20 text-rose',
    cardClass: 'text-rose',
  },
}

export default function AcademicsPage() {
  const searchParams = useSearchParams()
  const isDemo = searchParams.get('demo') === 'true'

  const rows: AcademicRow[] = isDemo ? buildDemoRows() : []

  const flaggedCount = rows.filter((r) => r.eligibility_status !== 'ok').length
  const eligibleCount = rows.filter((r) => r.eligibility_status === 'ok').length

  return (
    <div className="flex flex-1 flex-col overflow-hidden">
      <TopBar
        title="Academic Eligibility"
        subtitle={`${eligibleCount} eligible · ${flaggedCount} flagged · ${rows.length} athletes`}
      />

      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        {/* Summary Cards */}
        <div className="grid grid-cols-3 gap-4">
          {(['ok', 'review', 'ineligible'] as const).map((status) => {
            const count = rows.filter((r) => r.eligibility_status === status).length
            const cfg = ELIGIBILITY_CONFIG[status]
            const Icon = cfg.icon
            return (
              <div
                key={status}
                className="rounded-xl border border-border-1 bg-surface-2 p-4 flex items-center gap-3"
              >
                <div className={`flex h-8 w-8 items-center justify-center rounded-lg border ${cfg.className}`}>
                  <Icon className="h-4 w-4" />
                </div>
                <div>
                  <div className={`text-2xl font-bold tabular-nums ${cfg.cardClass}`}>{count}</div>
                  <div className="text-[10px] text-text-muted">{cfg.label}</div>
                </div>
              </div>
            )
          })}
        </div>

        {/* Privacy Notice */}
        <div className="rounded-xl border border-border-1 bg-surface-1 px-5 py-3">
          <p className="text-[11px] text-text-muted">
            <span className="font-semibold text-text-primary">Privacy:</span>{' '}
            Individual GPA and attendance figures are not displayed. Coaching staff can see eligibility
            status and flag reasons only — detailed academic records are held by the registrar&apos;s office.
          </p>
        </div>

        {rows.length === 0 ? (
          <EmptyState
            icon="🎓"
            title="No academic records"
            description="Academic eligibility records will appear here once they are added for your athletes."
          />
        ) : (
          <div className="rounded-xl border border-border-1 bg-surface-2 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border-1 bg-surface-3/50">
                    <th className="px-5 py-3 text-left text-xs font-medium text-text-muted">Athlete</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-text-muted">Eligibility</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-text-muted">Flag Note</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-text-muted">Last Updated</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border-1">
                  {rows
                    .slice()
                    .sort((a, b) => {
                      const order = { ineligible: 0, review: 1, ok: 2 }
                      return order[a.eligibility_status] - order[b.eligibility_status]
                    })
                    .map((row) => {
                      const cfg = ELIGIBILITY_CONFIG[row.eligibility_status]
                      return (
                        <tr key={row.id} className="hover:bg-surface-3/40 transition-colors">
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                              <div
                                className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold"
                                style={{
                                  backgroundColor: row.athlete.avatar_color ?? '#8B8FA8',
                                  color: '#0F1117',
                                }}
                              >
                                {row.athlete.initials ?? '?'}
                              </div>
                              <div>
                                <div className="text-xs font-medium text-text-primary">
                                  {row.athlete.full_name}
                                </div>
                                <div className="text-[10px] text-text-muted">
                                  {row.athlete.position} · {row.athlete.year_of_study}
                                </div>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex items-center rounded border px-2 py-0.5 text-xs font-semibold ${cfg.className}`}
                            >
                              {cfg.label}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-xs text-text-muted max-w-[280px]">
                            {row.flag_reason ? (
                              <div className="flex items-start gap-1.5">
                                <FlagTag type="academic" />
                                <span>{row.flag_reason}</span>
                              </div>
                            ) : (
                              <span className="text-text-faint italic">No flags</span>
                            )}
                          </td>
                          <td className="px-4 py-3 text-right text-xs text-text-muted whitespace-nowrap">
                            {new Date(row.updated_at).toLocaleDateString('en-GB', {
                              day: 'numeric',
                              month: 'short',
                              year: 'numeric',
                            })}
                          </td>
                        </tr>
                      )
                    })}
                </tbody>
              </table>
            </div>
          </div>
        )}

        <div className="flex items-start gap-2 rounded-xl border border-border-1 bg-surface-1 px-5 py-3">
          <BookOpen className="mt-0.5 h-4 w-4 shrink-0 text-text-faint" />
          <p className="text-[11px] text-text-muted">
            Athletes with <span className="font-semibold text-amber">Under Review</span> or{' '}
            <span className="font-semibold text-rose">Ineligible</span> status should be referred to the
            academic advisor and compliance officer before their next competitive fixture.
          </p>
        </div>
      </div>
    </div>
  )
}
