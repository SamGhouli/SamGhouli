...
// Sessions and matches in the past 7 days (derived from demo data)
const RECENT_SESSIONS = DEMO_DATA.trainingSessions.length
const UPCOMING_SESSIONS = DEMO_DATA.upcomingEvents.filter(
  (e) => e.event_type === 'training'
).length
const UPCOMING_MATCHES = DEMO_DATA.upcomingEvents.filter(
  (e) => e.event_type === 'match'
).length
const RECENT_MATCHES = DEMO_DATA.matchStats
  .map((m) => m.match_date)
  .filter((v, i, a) => a.indexOf(v) === i).length

// Per-athlete mock sessions count (7-day)
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        {loading ? (
          ...
          <>
            ...
            {/* Bar Chart: Readiness per athlete */}
            ...
            {/* Line Chart: 7-day trend (readiness + load) */}
            <div className="rounded-xl border border-border-1 bg-surface-2 p-5">
              <h2 className="mb-4 text-sm font-semibold text-text-primary">
              ...
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={trendData} margin={{ top: 4, right: 12, bottom: 4, left: 0 }}>
                  ...
                  <Line
                    ...
                    dataKey="readiness"
                    name="Avg Readiness"
                    stroke={CHART_COLORS.readiness}
                    strokeWidth={2}
                    dot={{ r: 3, fill: CHART_COLORS.readiness }}
                    activeDot={{ r: 5 }}
                  />
                  <Line
                    type="monotone"
                    ...
                    strokeWidth={2}
                    strokeDasharray="4 2"
                    dot={{ r: 3, fill: CHART_COLORS.load }}
                    activeDot={{ r: 5 }}
                  />
                </LineChart>
              </ResponsiveContainer>
                        ...
                        return (
                          <tr key={athlete.id} className="hover:bg-surface-3/50 transition-colors">
                            <td className="py-3">
                              <div className="flex items-center gap-2">
                                <div
                                  ...
                                  style={{
                                    backgroundColor: athlete.user?.avatar_color ?? '#8B8FA8',
                                    color: '#0F1117',
                                  }}
                                >
                            ...
                            <td className="py-3 text-right text-xs text-text-muted">
                              {athlete.training_load.toFixed(1)} AU
                            </td>
                            <td className="py-3 text-right text-xs text-text-muted">
                              {sessions}
                            </td>
                            <td className="py-3 text-right">
                              <StatusDot status={athlete.availability?.status ?? 'full'} showLabel />
                            </td>
                            <td className="py-3 text-right">
                              <span
                                className={`inline-flex items-center rounded border px-2 py-0.5 text-[10px] font-semibold ${risk.className}`}
                              >
                                {risk.label}
                              </span>
                            </td>
